...
let hardMode = 60;
  let mineArray = [];
    let mine = "<img src='mine.jpg' height='10' width='10'>";
...
function gameTime(){
    let table = document.getElementById("Game");
      for (i=0;i<256;i++){ // Assign a value from 0-255 to every button generated by the hacky script above.
  ...
  for (i=0; i<x; i++){
  mineArray.push(Math.round(Math.random() * 255)); // Fixed to 255 thanks to Chris.
}
};
  mineField(easyMode); // Hardcoding easy mode state for now. This will be built out with a selector in final version.
  ...
  function isBoom(x) {
    ...
    let inButt = parseInt(x.target.id);
      let dicks = document.getElementById(document.getElementById(inButt).parentNode.id).getElementsByTagName('tr')[0];
      dicks.setAttribute("id", "");
    if (mineArray.includes(inButt)){
      let makeBoom = document.getElementById(document.getElementById(inButt).parentNode.id);
        let killWord = document.getElementById("title");
        killWord.innerHTML = "You ded -- try again!";
          makeBoom.innerHTML = mine;
    } else {
      console.log("The value of inbutt is " + inButt);
      let notMine = document.getElementById(document.getElementById(inButt).parentNode.id);
      console.log(document.getElementById(inButt).parentNode);
      console.log(document.getElementById(document.getElementById(inButt).parentNode.id).getElementsByTagName('tr')[0]);
      console.log(notMine.id.split("").splice(1,3));
        let mineCount = [];
        mineCount.push(inButt); // Push the values of the buttons into an array and compare it. This should really cycle through and test an include and increment something
